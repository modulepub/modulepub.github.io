...
生产环境 Actuator、网关与网络隔离见仓库根 [运维说明](/en/dev/tg-boot/ops/)。
...
新增消费者须在 [跨模块协作规范](/en/dev/tg-boot/cross-module-collaboration/) 登记 **幂等键**。
...
- **跨模块**协作规则见 [跨模块协作规范](/en/dev/tg-boot/cross-module-collaboration/)。
- **模块内**多实现在 **`*-biz/service`** 定义 SPI，由注册表或 Spring 注入 `List<实现>` 路由。

## 架构测试

模块边界由 [架构测试门禁](/en/dev/tg-boot/architecture-tests/) 在 **Maven 测试阶段** 守护（ArchUnit + POM 扫描）；该模块不属于系统运行时代码，不参与 Spring Boot 启动。

```bash
mvn -pl spring-boot-starter-architecture-tests -am test
```
**源码路径**：`tg-boot/spring-boot-starter-module/spring-boot-starter-common/`