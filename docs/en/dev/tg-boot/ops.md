---
title: TG-boot Operations
createTime: 2026/06/02
permalink: /en/dev/tg-boot/ops/
---
# TG-boot 运维说明

面向部署、监控与生产环境配置。开发联调见根目录 [TG-boot 总览](/en/dev/tg-boot/)；跨模块 MQ 协作见 [跨模块协作规范](/en/dev/tg-boot/cross-module-collaboration/)。
...
- 目的地与幂等约定：[跨模块协作规范](/en/dev/tg-boot/cross-module-collaboration/)（含 `DLQ.*` 队列深度告警）
