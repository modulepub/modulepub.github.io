---
title: TG-boot AI Coding Guide
createTime: 2026/06/02
permalink: /en/dev/tg-boot/agents/
---
# TG-boot AI 编码规范（必读）

> 面向 Cursor / Copilot 等 AI 助手。**生成或修改本仓库 Java / Vue 代码前，请先阅读本文**；详细背景见 [TG-boot 总览](/en/dev/tg-boot/)，跨模块规则见 [跨模块协作规范](/en/dev/tg-boot/cross-module-collaboration/)。
...
1. 本文（硬性约定 + 正反例）
2. [TG-boot 总览](/en/dev/tg-boot/)（架构、字典/状态机、模块树）
...
- [ ] 枚举与 `*StatusCode` / `*ProcessCode` 符合 README 约定
- [ ] 未在 `*-biz` 直接依赖其他域 `*-biz`
...
| 运维 / Actuator | [运维说明](/en/dev/tg-boot/ops/)（生产禁止公网暴露 `/actuator/**`） |
| 管理端前端 | `tg-vue/` |
