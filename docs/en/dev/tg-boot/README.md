...
**AI 生成代码前请先读**：[AI 编码规范](/en/dev/tg-boot/agents/)（实体 `BaseEntity`、双主键、`getByCode` 等硬性约定与检查清单）。

**部署与生产运维**：[运维说明](/en/dev/tg-boot/ops/)（含 **Actuator 禁止对公网暴露**、MQ、架构测试门禁）。
...
| 分区 | 说明 |
|------|------|
| [AI 编码规范](/en/dev/tg-boot/agents/) | 实体、命名、跨模块硬性约定（生成代码前必读） |
| [运维说明](/en/dev/tg-boot/ops/) | Actuator 安全、MQ、上线自查 |
| [跨模块协作规范](/en/dev/tg-boot/cross-module-collaboration/) | MQ 与同步 `Api**Service` 选用规则 |
| [架构测试门禁](/en/dev/tg-boot/architecture-tests/) | ArchUnit 模块边界守护 |
| [树-图架构](/en/dev/tg-boot/tree-graph-architecture/) | 树形定边界、图形定关系 |
| [starter-module](/en/dev/tg-boot/starter-module/) | Maven 聚合根 |
| [runner](/en/dev/tg-boot/runner/) | 默认单体启动器 |
| [plugins](/en/dev/tg-boot/plugins/) | 运行时外置插件目录 |
| [components](/en/dev/tg-boot/components/) | 通用 Starter |
| [business](/en/dev/tg-boot/business/) | 垂直业务域 |
| [tg-manage-vue](/en/dev/tg-boot/frontend-tg-manage-vue/) | 管理端前端 |

中文版入口：[TG-boot 总览](/dev/tg-boot/)。
