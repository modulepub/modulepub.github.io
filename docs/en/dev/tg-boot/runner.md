...
- **生产环境**：Actuator（`/actuator/**`）**不得**对公网或不可信网络暴露；须通过网关、防火墙或独立内网管理端口收敛访问面。详见仓库根 [运维说明](/en/dev/tg-boot/ops/)。
...
- 运维（Actuator、MQ、上线自查）：[运维说明](/en/dev/tg-boot/ops/)
- 模块总览：[starter-module](/en/dev/tg-boot/starter-module/)
- MQ 目的地总表：仓库根 [TG-boot 总览](/en/dev/tg-boot/)
